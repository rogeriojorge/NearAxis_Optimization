...
def get_stel(ind,nphi=251):
    ...
    if ind==11:
        name   = 'QH_NFP5_plasma'
        rc     = [ 1.0,0.1521724412489352,0.008608686214203631,-0.0015335148850080676,-0.00034396026617238533,1.2821750944353719e-05,1.1440867803187253e-05,8.841412711585242e-07 ]
        zs     = [ 0.0,0.12529361440704864,0.007669807678622627,-0.0013091238174076114,-0.00031332403362836113,1.0857150512359865e-05,1.0951343985342566e-05,3.071252875189507e-07 ]
        etabar =  2.79069698552972
        nfp    =  5
        B2c    =  0.34939577005839684
        p2     =  -1000000.0
        iota   =  -1.4693209309843993
        stel   =  make_optimizable(Qsc(rc=rc, zs=zs, etabar=etabar, nfp=nfp, nphi=nphi, B2c=B2c, order='r3', p2=p2))
        r_edge =  1/12
        coilSeparation = 0.15
        targetValue = 0.08
        nCoilsPerNFP = 4
        # DMerc mean  =  0.028863860708905564
        # DWell mean  =  0.572427280839828
        # Max elongation  =  2.5025907355058883
        # B20 variation = 0.06949335628666375
        # Max |X20| = 3.3893707172774414
        # Max |X3c1| = 0.9531405541680226
        # gradgradB inverse length:  5.0900594797068734
        # objective function:  3400.1268876839627
