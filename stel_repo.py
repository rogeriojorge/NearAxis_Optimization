...
def get_stel(ind,nphi=251,r_edge=0.06,coilSeparation = 0.1,targetValue = 0.08,nCoilsPerNFP = 6):
    ...
    if ind==24:
        ...
        zs      = [ 0.0,0.0,-0.0163514270609143,0.0,-0.0015042879570196547,0.0,-0.00021700040559117844,0.0,6.059403577702251e-05,0.0,-5.452076952577057e-05,0.0,3.328867252080692e-05,0.0,-1.0299487306513586e-05,0.0,2.137923805957748e-06 ]
        B0_vals = [ 1.0,0.10419385079223667 ]
        d_svals = [ 0.0,0.9922744118926163,0.030690466036873903,-0.028300506358705273,0.04243709473306234,-0.0043252236376582496,-0.003102883102965302,-6.178176581030303e-05,5.732402026008092e-06 ]
        delta   = 0.4970970269996152
        nfp     = 2
        stel    =  make_optimizable(Qsc(rc=rc,zs=zs, nfp=nfp, B0_vals=B0_vals, d_svals=d_svals, nphi=nphi, omn=True, delta=delta))
        iota    =  -3.0143228929246244
        # mean gradB inverse length: 2.503913066657332
        # Max elongation = 5.4705557112372505
        # objective function:  561.3541592738825
    if ind==25:
        name   = 'QI_NFP3_r1'
